      ...
      <div>
        ...
        <div className="max-w-5xl mx-auto px-6 sm:px-8 py-12 space-y-8">
          <div className="p-8 rounded-3xl bg-white border border-zinc-200/80 shadow-xs space-y-6">
            ...
            {/* Campaign Requirements & Vetting Info */}
            <div className="p-6 rounded-2xl bg-zinc-50 border border-zinc-200 space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-900 flex items-center gap-2">
                ...
                Vetting Criteria & Allocation Rules (Set by Project)
              </h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                <div className="p-3 bg-white border border-zinc-200 rounded-xl">
                  <span className="text-zinc-500 block text-[11px]">Spots Per Community</span>
                  <span className="font-bold text-emerald-700 text-sm">{campaignData.spotsPerCommunity || 10} WL Spots</span>
                </div>

                <div className="p-3 bg-white border border-zinc-200 rounded-xl">
                  <span className="text-zinc-500 block text-[11px]">Min Discord Members</span>
                  <span className="font-bold text-zinc-900 text-sm">
                    {campaignData.minDiscordMembers ? campaignData.minDiscordMembers.toLocaleString() : "None"}
                  </span>
                </div>

                <div className="p-3 bg-white border border-zinc-200 rounded-xl">
                  <span className="text-zinc-500 block text-[11px]">Min X Followers</span>
                  <span className="font-bold text-zinc-900 text-sm">
                    {campaignData.minXFollowers ? campaignData.minXFollowers.toLocaleString() : "None"}
                  </span>
                </div>

                <div className="p-3 bg-white border border-zinc-200 rounded-xl">
                  <span className="text-zinc-500 block text-[11px]">Social Connections</span>
                  <span className="font-bold text-zinc-900 text-sm">
                    {campaignData.requireDiscordVerification && campaignData.requireXVerification
                      ? "Discord & X Required"
                      : campaignData.requireDiscordVerification
                      ? "Discord Required"
                      : campaignData.requireXVerification
                      ? "X Required"
                      : "Optional"}
                  </span>
                </div>
              ...
              {campaignData.customRequirements && (
                <div className="pt-2 text-xs text-zinc-700 font-medium">
                  <strong>Custom Rules:</strong> {campaignData.customRequirements}
                </div>
              )}
            </div>
            ...
            <div className="pt-4 flex flex-wrap items-center gap-4">
              ...
              {!isCampaignClosed ? (
                ...
                >
                  ...
                  <span>Apply for DAO Allocation ({campaignData.spotsPerCommunity || 10} Spots)</span>
                </button>
              ) : (
                <button
            ...
            >
              ...
              {modalMode === "collab_app" && (
                <form onSubmit={handleApplySubmit} className="space-y-4">
                  {/* Verified Platform Profile Metrics Container */}
                  <div className="p-4 bg-zinc-50 border border-zinc-200/90 rounded-2xl space-y-3">
                    <div className="flex items-center justify-between border-b border-zinc-200 pb-2">
                      <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-900 uppercase tracking-wider">
                        ...
                        <span>Polled Platform Metrics</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2.5 text-xs">
                      <div className="p-2.5 bg-white border border-zinc-200 rounded-xl">
                        <span className="text-zinc-400 text-[10px] block">Represented Community</span>
                        <span className="font-bold text-zinc-900">{appForm.communityName || "Alpha Seekers DAO"}</span>
                      </div>

                      <div className="p-2.5 bg-white border border-zinc-200 rounded-xl">
                        <span className="text-zinc-400 text-[10px] block">WL Spots Allocated</span>
                        <span className="font-bold text-emerald-700">{campaignData.spotsPerCommunity || 10} Spots (Fixed)</span>
                      </div>

                      <div className="p-2.5 bg-white border border-zinc-200 rounded-xl">
                        <span className="text-zinc-400 text-[10px] block">Discord Members</span>
                        <span className="font-bold text-indigo-900">{(parseInt(appForm.memberCount, 10) || 12500).toLocaleString()} Members</span>
                      </div>

                      <div className="p-2.5 bg-white border border-zinc-200 rounded-xl">
                        <span className="text-zinc-400 text-[10px] block">X Followers</span>
                        <span className="font-bold text-zinc-900">{(parseInt(appForm.xFollowerCount, 10) || 45000).toLocaleString()} Followers</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-3.5 py-2.5 bg-white border border-zinc-200 rounded-xl text-xs font-medium focus:outline-none focus:border-zinc-900 resize-none"
                    />
                  </div>
