...
export default async function CollabManagerProfilePage({ params }: ProfilePageProps) {
  const resolvedParams = await params
  const rawUsername = resolvedParams.username || "collabmanager"
  // Clean leading '@' if present in URL (e.g., /@alex -> alex)
  const username = decodeURIComponent(rawUsername).replace(/^@/, "")
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 selection:bg-zinc-100 selection:text-zinc-900 flex flex-col justify-between">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <div className="relative -mt-16 mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pb-6 border-b border-zinc-100">
            <div className="flex items-end gap-5">
              {/* Avatar */}
              <div className="w-28 h-28 rounded-2xl bg-zinc-900 text-white font-bold text-3xl flex items-center justify-center border-4 border-white shadow-md uppercase">
                {username.slice(0, 2)}
              </div>
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-bold text-zinc-900 tracking-tight capitalize">
                    {username}
                  </h1>
            ...
            <div className="flex items-center gap-3">
              <Link
                href="/sign-in"
                className="px-4 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white text-sm font-medium rounded-xl shadow-xs transition-colors cursor-pointer"
              >
          ...
          <div className="space-y-6">
            <div className="prose prose-zinc max-w-none">
              <p className="text-zinc-600 text-sm sm:text-base leading-relaxed">
                Managing community collaborations, whitelist spot allocations, and partner giveaways for <strong>@{username}</strong>. 
            ...
            {/* Open Collaborations Section Placeholder */}
              ...
              <div className="p-8 text-center rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/50 space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-zinc-500 max-w-sm mx-auto">
                    Check back soon or sign in to send a direct collaboration inquiry to @{username}.
                  </p>
                </div>
              </div>
            </div>
      ...
      <footer className="border-t border-zinc-100 mt-12 bg-white">
        <PoweredBadge />
      </footer>
    </div>
  )
