...
import { Handshake, CheckCircle2, Briefcase, FileSpreadsheet, AlertCircle, ExternalLink } from "lucide-react"
...
interface CmCollaborationsClientProps {
  initialCollaborations: any[]
}

export function CmCollaborationsClient({ initialCollaborations = [] }: CmCollaborationsClientProps) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200/80 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-zinc-500 font-normal">
            Submit member wallets inside the platform to complete approved allocation deals.
          </p>
        </div>
      ...
      <div className="p-6 bg-white border border-zinc-200/80 shadow-2xs space-y-6">
        <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
          <div className="flex items-center gap-2 text-xs font-bold text-zinc-900">
            ...
            <span>Active Managed Deals ({collaborations.length})</span>
          </div>
        </div>
        ...
        <div className="divide-y divide-zinc-100">
          {collaborations.length > 0 ? (
            collaborations.map((collab) => {
              ...
              return (
                <div key={collab.id} className="py-6 space-y-4">
                    ...
                    <div className="space-y-1.5 min-w-0">
                      <div className="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span className="px-2.5 py-0.5 text-[11px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-md">
                          ✓ {collab.allocatedSpots || 10} Spots Secured
                        </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0 flex-wrap">
                      ...
                      {isCompleted ? (
                        <span className="px-3 py-1 text-xs font-bold bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-xl flex items-center gap-1.5">
                          ...
                          <span>Deal Fully Completed ({claimed}/{total})</span>
                        </span>
                      ) : (
                  ...
                  {/* Allocation Performance Bar */}
                  <div className="p-4 bg-zinc-50 border border-zinc-100 space-y-2 rounded-xl">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-zinc-700">Wallet Submission Progress</span>
            ...
            <div className="py-12 text-center space-y-3">
              ...
              <p className="text-sm font-semibold text-zinc-700">No active managed deals yet</p>
              <p className="text-xs text-zinc-400">
                Pitch open client campaigns to close whitelist allocation deals and build your manager portfolio.
              </p>
            </div>
          )}
        </div>
